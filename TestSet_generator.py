...
for dirname, _, filenames in os.walk('Revitsone-5classes-test'):
    for filename in filenames:
        os.path.join(dirname, filename)

# dir path
BASE_DIR = 'Revitsone-5classes-test/'
...
image = mpimg.imread(other_activities + df['image_id'].iloc[0])
plt.imshow(image)
plt.show()

df.to_csv('driver_behavior_test.csv', index=True)
